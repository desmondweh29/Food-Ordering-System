...
function addIntoCart($conn,$accountID,$name,$quantity,$category,$price)
{
    $sql = "INSERT INTO cart (accountID, M_Name, M_Quantity, M_Category, M_Price) VALUES (?,?,?,?,?) ;";
    $stmt = mysqli_stmt_init($conn);
    ...
    mysqli_stmt_bind_param($stmt,"sssss",$accountID,$name,$quantity,$category,$price);
    mysqli_stmt_execute($stmt);    
    mysqli_stmt_close($stmt);
